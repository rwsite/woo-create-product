...
function variables_and_variations(WC_Product $product, $data)
{
    ...
    foreach ($data['attributes'] as $taxonomy) {
        ...
        $taxonomy_name     = wc_attribute_taxonomy_name( mb_strimwidth($taxonomy['name'], 0, 20) ); // 'pa_' . sanitize_title( 'Цвет');
        ...
        foreach ($taxonomy['options'] as $term_value) {
            ...
            if (taxonomy_exists($taxonomy_name)) {
                $result = wp_set_object_terms($product->get_id(), [$term->term_id], $taxonomy_name, true); // после установки аттрибута
